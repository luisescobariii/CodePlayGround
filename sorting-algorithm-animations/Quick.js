class Quick extends Algorithm {
    ...
    constructor() {
        super();
        this.stack = [];
        this.stack.push({ left: 0, right: count - 1 });
        this.pending = false;
    ...
    partition(left, right) {
        ...
        this.index = values[Math.floor((this.left + this.right) / 2)];
        this.swaped = this.left;
        
        this.checks++;
        if (this.left <= this.right) {
            this.checks++;
            while (values[this.left] < this.index) {
                this.checks++;
                this.left++;
            }
            this.checks++;
            while (values[this.right] > this.index) {
                this.checks++;
                this.right--;
            }
